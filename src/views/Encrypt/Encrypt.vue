<template>
  <div class="encrypt-container">
    ...
    <section class="action-section">
      <button :disabled="!canStart || isEncrypting" @click="startEncryption">Titkosítás Indítása</button>
    </section>

    <section v-if="encryptionStatus" class="status-section">
      <h2>Titkosítás Állapota</h2>
      <p>Fájl: {{ currentFileName }}</p>
      <p>Státusz: {{ encryptionStatus.status }}</p>
      <p v-if="encryptionStatus.percentage">Állapot: {{ encryptionStatus.percentage }} / 100</p>
    </section>
  </div>
</template>
  ...
  methods: {
    ...
    async fetchEncryptions() {
      try {
        ...
        dialogs.showError("Hiba a titkosítási módszerek lekérésekor: Részletek a logban")
    ...
    async fetchStatus() {
      ...
      try {
        ...
        } else if (progress === "Done" || progress === "Failed" || progress === "Not started") {
        ...
        dialogs.showError("Hiba a titkosítás állapotának lekérésekor: Részletek a logban")
    ...
    async startEncryption() {
      try {
        ...
        const response = await axios.post("http://localhost:8001/startencrypt", payload);
        ...
        dialogs.showError("Hiba a titkosítás indításakor: Részletek a logban")
